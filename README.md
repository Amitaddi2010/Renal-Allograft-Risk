...
- **Structure**: a landing page (hero, two feature cards, how-it-works, KPI tiles, collapsible study evidence) and a dashboard with a home page plus two modules reached from the sidebar: **Risk calculator** and **HLA & eplets**. The dashboard home shows module shortcuts, recent HLA analyses and recent risk estimates (saved in the browser with *Save to recent* / *Save estimate*, reopened with one click) and reference-data status. The default look is a light clinical dashboard (`dashboard_theme.css`); the original dark "terminal" look can be switched on from the help dock.
