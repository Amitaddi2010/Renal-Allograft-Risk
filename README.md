...
- **Structure**: two pages. The **landing page** is a single hero section with an animated 3D DNA helix (`dna_hero.js`, plain 2D-canvas perspective renderer, no library; a static frame under the reduced-motion setting) and two buttons: *Open dashboard* and *Try the example pair*. The **dashboard** is a frosted-glass layout (`dashboard_theme.css`) with a sidebar (Dashboard, Risk calculator, HLA & eplets, Back to site), a summary strip (antigen mismatches, mismatched eplets, immunogenic eplets, predicted risk) and a home page with module shortcuts, recent HLA analyses and risk estimates (saved in the browser with *Save to recent* / *Save estimate*), reference-data status and the collapsible study evidence. Deep links: `#dashboard`, `#risk`, `#hla`, `#hla-example`, `#hla-3d-example`. The original dark "terminal" look can still be switched on from the help dock. `tools/rebuild_views.py` regenerates the landing page, summary strip and home pane idempotently.
